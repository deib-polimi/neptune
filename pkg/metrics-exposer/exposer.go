...
// functionMetricsProvider is a sample implementation of provider.MetricsProvider which stores a map of fake metrics
type functionMetricsProvider struct {
...
// NewFunctionMetricsProvider returns an instance of responseTimeMetricsProvider
func NewFunctionMetricsProvider(client dynamic.Interface, mapper apimeta.RESTMapper, stopCh <-chan struct{}) provider.CustomMetricsProvider {
	p := &functionMetricsProvider{
		client: client,
		mapper: mapper,
		cache:  make(map[CustomMetricResource]metricValue),
	}

	// TODO: update metrics periodically
...
func (p *functionMetricsProvider) valueFor(info provider.CustomMetricInfo, namespacedName types.NamespacedName, metricSelector labels.Selector) (resource.Quantity, error) {
...
func (p *functionMetricsProvider) metricFor(value resource.Quantity, name types.NamespacedName, selector labels.Selector, info provider.CustomMetricInfo, metricSelector labels.Selector) (*custom_metrics.MetricValue, error) {
...
func (p *functionMetricsProvider) metricsFor(namespace string, selector labels.Selector, info provider.CustomMetricInfo, metricSelector labels.Selector) (*custom_metrics.MetricValueList, error) {
...
func (p *functionMetricsProvider) GetMetricByName(name types.NamespacedName, info provider.CustomMetricInfo, metricSelector labels.Selector) (*custom_metrics.MetricValue, error) {
...
func (p *functionMetricsProvider) GetMetricBySelector(namespace string, selector labels.Selector, info provider.CustomMetricInfo, metricSelector labels.Selector) (*custom_metrics.MetricValueList, error) {
...
func (p *functionMetricsProvider) ListAllMetrics() []provider.CustomMetricInfo {
